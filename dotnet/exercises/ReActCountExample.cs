...
/// </summary>
public static class ReActCountExample
{
...
Answer: The capital of France is Paris";
    ...
    private static Task<string> CountLettersAsync(string query)
    {
        // Count occurrences of a letter in a word.
        // Expected formats (case-insensitive):
        //   "letter X in word Y" or "letter X in Y" or "X in word Y" or "X in Y"
        if (string.IsNullOrWhiteSpace(query))
        {
            return Task.FromResult("Please provide a query, e.g. 'letter r in word strawberry'.");
        }

        var original = query.Trim();
        var q = original.ToLowerInvariant();

        var patterns = new[]
        {
            new Regex(@"^letter\s+(\w)\s+in\s+word\s+(\w+)$", RegexOptions.Compiled),
            new Regex(@"^letter\s+(\w)\s+in\s+(\w+)$", RegexOptions.Compiled),
            new Regex(@"^(\w)\s+in\s+word\s+(\w+)$", RegexOptions.Compiled),
            new Regex(@"^(\w)\s+in\s+(\w+)$", RegexOptions.Compiled)
        };
        ...
        foreach (var rx in patterns)
        {
            ...
            if (m.Success)
            {
                ...
                if (letter.Length != 1)
                {
                    return Task.FromResult("Please specify a single letter to count.");
                }
                ...
                return Task.FromResult($"The letter '{letter}' appears {count} time(s) in the word '{word}'");
            }
        }

        return Task.FromResult($"Could not parse the query '{original}'. Please use format like 'letter r in word strawberry' or 'r in strawberry'");
    }
    ...
    private static readonly Dictionary<string, Func<string, Task<string>>> KnownActions = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["count_letters"] = CountLettersAsync
    };

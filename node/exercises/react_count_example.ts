...
function countLetters(query: string): string {
    /**
    ...
    for (const pattern of patterns) {
        ...
        if (match) {
            const [, letter, word] = match;
            const count = (word.match(new RegExp(letter, 'g')) || []).length;
            return `The letter '${letter}' appears ${count} time(s) in the word '${word}'`;
        }
    }
    
    return `Could not parse the query '${query}'. Please use format like 'letter r in word strawberry' or 'r in strawberry'`;
}

const knownActions: { [key: string]: (input: string) => Promise<string> | string } = {
    wikipedia,
    calculate,
    count_letters: countLetters
};
...
export { ChatBot, query, wikipedia, calculate, countLetters, knownActions };
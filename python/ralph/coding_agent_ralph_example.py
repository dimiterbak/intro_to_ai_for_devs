...
# Add parent directory to path to allow importing coding_agent_tools
...
class ChatBot:
    ...
    def __call__(self, message):
        ...
        # Return the response message object for agent_loop() to handle tool calling
    ...
    def execute(self):
        ...
        try:
            ...
            # Re-raise KeyboardInterrupt so it can be handled at the top level
            raise
        except Exception as e:
    ...
    try:
        while True:
            ...
            if user_query:  # Only process non-empty queries
                result = agent_loop(user_query, bot)
...
if __name__ == '__main__':
    try:
        ...
        # Check if stdin has input (for piping, e.g., from ralph.sh)
            ...
            # No arguments and no stdin input - start interactive mode
            interactive_loop()
    except KeyboardInterrupt:
        ...
        # Handle broken pipe (e.g., when output is piped and reader closes)
